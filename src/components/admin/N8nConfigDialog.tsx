...
export function N8nConfigDialog({ open, onOpenChange }: N8nConfigDialogProps) {
  ...
  "nodes": [
    ...
    {
      "name": "Enviar Texto Z-API",
      "type": "n8n-nodes-base.httpRequest",
      "typeVersion": 1,
      "position": [650, 200],
      "parameters": {
        "method": "POST",
        "url": "=https://api.z-api.io/instances/{{ $('Webhook Envio').item.json.instanceId }}/token/{{ $('Webhook Envio').item.json.token }}/send-text",
        "headers": {
          "Client-Token": "={{ $('Webhook Envio').item.json.token }}"
        },
        "jsonParameters": true,
        "parametersJson": "={ \\"phone\\": \\"{{ $json.body.phone }}\\", \\"message\\": \\"{{ $json.body.data.message }}\\" }"
      }
    },
    {
      "name": "Enviar Imagem Z-API",
      "type": "n8n-nodes-base.httpRequest",
      "typeVersion": 1,
      "position": [650, 300],
      "parameters": {
        "method": "POST",
        "url": "=https://api.z-api.io/instances/{{ $('Webhook Envio').item.json.instanceId }}/token/{{ $('Webhook Envio').item.json.token }}/send-image",
        "headers": {
          "Client-Token": "={{ $('Webhook Envio').item.json.token }}"
        },
        "jsonParameters": true,
        "parametersJson": "={ \\"phone\\": \\"{{ $json.body.phone }}\\", \\"image\\": \\"{{ $json.body.data.imageUrl }}\\", \\"caption\\": \\"{{ $json.body.data.caption }}\\" }"
      }
    },
    {
      "name": "Enviar Documento Z-API",
      "type": "n8n-nodes-base.httpRequest",
      "typeVersion": 1,
      "position": [650, 400],
      "parameters": {
        "method": "POST",
        "url": "=https://api.z-api.io/instances/{{ $('Webhook Envio').item.json.instanceId }}/token/{{ $('Webhook Envio').item.json.token }}/send-document-url",
        "headers": {
          "Client-Token": "={{ $('Webhook Envio').item.json.token }}"
        },
        "jsonParameters": true,
        "parametersJson": "={ \\"phone\\": \\"{{ $json.body.phone }}\\", \\"document\\": \\"{{ $json.body.data.documentUrl }}\\", \\"fileName\\": \\"{{ $json.body.data.filename }}\\" }"
      }
    }
  ],
  "connections": {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure a integração entre n8n e Z-API para gerenciar mensagens do WhatsApp
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-600">
                    Cliente → Z-API → n8n → Amplie Chat
                  </p>
                </div>
                <div className="space-y-2">
                  <Badge variant="outline" className="bg-blue-50">Envio</Badge>
                  <p className="text-sm text-gray-600">
                    Amplie Chat → n8n → Z-API → Cliente
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Crie este workflow no n8n para receber mensagens da Z-API
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Crie este workflow no n8n para enviar mensagens via Z-API
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="zapi-instance">Instance ID da Z-API</Label>
                  <Input
                    id="zapi-instance"
                    value={zapiInstanceId}
                    onChange={(e) => setZapiInstanceId(e.target.value)}
                    placeholder="Ex: 3C4DBD4C5..."
                  />
                </div>
                <div>
                  <Label htmlFor="zapi-token">Token da Z-API</Label>
                  <Input
                    id="zapi-token"
                    type="password"
                    value={zapiToken}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure o webhook na Z-API para apontar para o n8n
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label>URL para Configurar na Z-API:</Label>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <label htmlFor="step3" className="text-sm">Webhook configurado na Z-API</label>
                </div>
                <div className="flex items-center space-x-2">
